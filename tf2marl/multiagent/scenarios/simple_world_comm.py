...
class Scenario(BaseScenario):
    ...
    def make_world(self):
        ...
        world.dim_c = 4 # communication dimension
        world.damping = 0.5 # velocity damping factor
        num_good_agents = 4 # number of good agents
        num_adversaries = 8 # number of adversaries
        num_agents = num_adversaries + num_good_agents # total number of agents
        num_landmarks = 4 # number of landmarks
        num_food = 4 # number of food
        num_forests = 4 # number of forests
        num_walls = 4 # number of walls
        # add agents
        world.agents = [Agent() for i in range(num_agents)]
        for i, agent in enumerate(world.agents):
            agent.name = 'agent %d' % i
            agent.collide = True # agents can collide with each other
            agent.leader = True if i == 0 else False # the first agent is the leader
            agent.silent = True if i > 0 else False # the leader can communicate, others are silent
            agent.adversary = True if i < num_adversaries else False # the first num_adversaries agents are adversaries
            agent.size = 0.075 if agent.adversary else 0.045 # adversaries are bigger than good agents
            agent.accel = 3.0 if agent.adversary else 4.0 # good agents have higher acceleration
            agent.max_speed = 1.0 if agent.adversary else 1.3 # good agents have higher max speed
            agent.color = np.array([0.85, 0.35, 0.35]) if agent.adversary else np.array([0.35, 0.85, 0.35]) # adversaries are red, good agents are green
        # add landmarks
        world.landmarks = [Landmark() for i in range(num_landmarks)]
        for i, landmark in enumerate(world.landmarks):
            landmark.name = 'landmark %d' % i
            landmark.collide = True # landmarks can collide with agents
            landmark.movable = False # landmarks are not movable
            landmark.size = 0.2 # landmarks are bigger than agents
            landmark.boundary = False # landmarks are not boundaries
            landmark.color = np.array([0.25, 0.25, 0.25]) # landmarks are gray
        # add food
        world.food = [Landmark() for i in range(num_food)]
        for i, landmark in enumerate(world.food):
            landmark.name = 'food %d' % i
            landmark.collide = False # food cannot collide with agents
            landmark.movable = False # food are not movable
            landmark.size = 0.03 # food are smaller than agents
            landmark.boundary = False # food are not boundaries
            landmark.color = np.array([0.35, 0.35, 0.85]) # food are blue
        # add forests
        world.forests = [Landmark() for i in range(num_forests)]
        for i, landmark in enumerate(world.forests):
            landmark.name = 'forest %d' % i
            landmark.collide = False # forests cannot collide with agents
            landmark.movable = False # forests are not movable
            landmark.size = 0.3 # forests are bigger than landmarks
            landmark.boundary = False # forests are not boundaries
            landmark.color = np.array([0.6, 0.9, 0.6]) # forests are light green
        # add walls
        world.walls = [Wall() for i in range(num_walls)]
        for i, wall in enumerate(world.walls):
            wall.name = 'wall %d' % i
            wall.orient = 'H' if i < 2 else 'V' # the first two walls are horizontal, the last two are vertical
            wall.axis_pos = -1.2 if i % 2 == 0 else 1.2 # the walls are located at the four sides of the world
            wall.start = -0.5 # the walls span from -0.5 to 0.5
            wall.end = 0.5
            wall.width = 0.2 # the walls have some width
            wall.hard = True # the walls are hard, cannot be penetrated
            wall.color = np.array([0.75, 0.75, 0.75]) # the walls are light gray
        # add all landmarks to the world
